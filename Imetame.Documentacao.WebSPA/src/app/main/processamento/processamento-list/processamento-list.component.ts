...
@Component({
    selector: "processamento-list",
    templateUrl: "./processamento-list.component.html",
    styleUrls: ["./processamento-list.component.scss"],
    encapsulation: ViewEncapsulation.None,
    animations: fuseAnimations,
})
export class ProcessamentoListComponent implements OnInit, OnDestroy, AfterViewInit {
    private _unsubscribeAll: Subject<any>;

    displayedColumns = ["ossString", "inicioProcessamento", "finalProcessamento", "status"];
    ...
    ngOnInit() {
        this.titleService.setTitle("Processamento - Imetame");

        this.paginator.page
            .pipe(takeUntil(this._unsubscribeAll))
            .subscribe((q) => {
                if (this.paginator.pageIndex != this.service.pageIndex) {
                    this.service.onPageIndexChanged.next(
                        this.paginator.pageIndex
                    );
                }

                if (this.paginator.pageSize != this.service.pageSize) {
                    this.service.onPageSizeChanged.next(
    ...
    abrir(item) {
        this.router.navigate(['/automacoes/cadastro-de-colaboradores', item.id]);
    }
