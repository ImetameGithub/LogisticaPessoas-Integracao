	...
	public class AtividadeEspecificaController : Controller
	{
		...
		public async Task<IActionResult> GetAllPaginated(int page = 1, int pageSize = 10, string filtro = "")
		{
			try
			{
				...
				IList<AtividadeEspecifica> listAtividadeEspecificas = await query.Skip(offset)
															 .Take(pageSize).ToListAsync();

				foreach (AtividadeEspecifica atividade in listAtividadeEspecificas)
				{
					atividade.QuantColaboradores = await _repColaboradorxAtividade.SelectContext()
																					.AsNoTracking()
																					.Where(x => x.CXA_IDATIVIDADE_ESPECIFICA.Equals(atividade.Id))
																					.CountAsync();
				}
